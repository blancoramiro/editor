...
const char* fragment_shader_linenumber_code =
	...
	"		if(ypos == 5.)"
	"		gl_FragColor = vec4(.5, 0.1, 0.1, 0.) * ceil(texture2D(Texture, vec2(((mod(gl_FragCoord.x, Grid.z)*14.)/Grid.z)/256.+(14./256.)*ceil(mod(Char,18.)), ((mod(gl_FragCoord.y+GridYOffset, Grid.w)*18.)/Grid.w)/128.+(18./128.)*  ceil(6.-Char/18.)     )));"
	"		else"
	"		gl_FragColor = vec4(.5, 0.1, 0.1, 0.) * ceil(texture2D(Texture, vec2(((mod(gl_FragCoord.x, Grid.z)*14.)/Grid.z)/256.+(14./256.)*ceil(mod(Char,18.)), ((mod(gl_FragCoord.y+GridYOffset, Grid.w)*18.)/Grid.w)/128.+(18./128.)*  ceil(6.-Char/18.)     )));"
#ifdef CHAR_GRID
	"	}"
